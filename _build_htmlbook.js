...
<div id="qa-player" role="region" aria-label="Audio">
  <div class="hd" title="Sudrab boshqa joyga qo'yish mumkin">
...
(function(){
  var box = document.getElementById("qa-player"), hd = box.querySelector(".hd"), frame = box.querySelector(".fr"),
      ttl = box.querySelector(".ttl"), cur = null, M = innerWidth < 400 ? 8 : 12, BAR = 44, pos = null, corner = 0;
  try { pos = JSON.parse(localStorage.getItem("qaPos") || "null"); } catch (e) {}
  function clamp(x, y){
  ...
  function play(btn){
    ...
    if (pos) place(pos.x, pos.y); else toCorner(0);
  }
  ...
  addEventListener("resize", function(){ if (pos && box.classList.contains("open")) place(pos.x, pos.y); });

  // drag by the title bar (mouse, pen or finger); the video itself stays clickable
  var start = null;
